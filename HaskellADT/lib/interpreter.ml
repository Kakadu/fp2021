...
module Interpret (M : MONADERROR) : sig 
  ...
  let rec pattern_decl_bindings pattern value = 
    ...
    | PCons (l, r), VList (hd::tl) -> 
      ...
      >>= fun hd_match -> 
      pattern_decl_bindings r (VList tl) 
  ...
  let rec check_pattern pattern value = 
    match (pattern, value) with
      ...
      | PNull, VList [] -> true
      | PTuple p, VTuple v ->
        List.length p = List.length v &&
        List.for_all2 (fun p v -> check_pattern p v) p v
      | _ -> false
      ...
      match expr with 
        ...
        | EBinOp (op, e1, e2) -> (
            ...
            match (value1, value2, op) with
              ...
              | VInt x, VInt y, Div -> if y = 0 then
                fail Division_by_zero else 
                  return (VInt (x / y))
        ...
        | ECase (expr, cases) -> (
          ...
          match List.find_opt (fun (pattern, _) -> check_pattern pattern value) cases 
          with
  ...
  and add_binding binding env = 
    match binding with
      | (pattern, expr) ->
            let* value = eval expr env in 
            let* binds = pattern_decl_bindings pattern value in
            return (exten_env env binds)
  let run program = 
    let* binds, _ = 
      List.fold_left
        (fun acc -> function
          | DAdt _ -> failwith "Error"
          | DLet binding ->
