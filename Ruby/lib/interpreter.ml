      ...
      let rec doer2 c s =
        ...
        then return c.last_return
          ...
          | hd :: tl ->
            ...
            | Expression e ->
              expr duo c e
              >>= fun x -> expr duo { c with signal = Return; last_return = x } e
        ...
        | Null ->
          ...
          | true ->
            ...
            doer1
              { (concat_var_lists
                   e_env
                   (combine_args_and_params (get_func_ctx_from_ctx i2 e_env).args x))
                with
                id = Identifier "tmp"
              ; gl_vars = e_env.vars
          ...
          | true ->
            ...
            | Identifier "call" ->
              ...
              | Lambda (il, sl) ->
                ...
                doer2
                  (concat_gl_var_lists
                     (concat_var_lists e_env (combine_args_and_params il x))
                     e_env.gl_vars)
                  sl
              | _ -> fail "lambda was expected")
            | Identifier "to_s" ->
              return
              @@ String (multliple_str_unpacker (get_var_ctx_from_ctx i e_env).value)
            | _ ->
              let obj_class =
                unpack_identifier_in_value (get_var_ctx_from_ctx i e_env).value
              in
              obj_class
              >>= fun x ->
    ...
    let rec stmt (duo : dispatch) (s_env : global_ctx) st =
      ...
      | Class (i, s) ->
        ...
        let cl_ctx =
          gl_ctx
          >>= fun gl_ctx ->
          return { tmp_class_ctx with vars = gl_ctx.vars; funcs = gl_ctx.funcs }
        in
        ...
        | false ->
          cl_ctx
          >>= fun cl_ctx ->
          return { s_env with classes = { cl_ctx with id = i } :: s_env.classes })
